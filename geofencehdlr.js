...
// ✅ EXPIRY FEATURE IMPORT (added)
import { computeExpiryAt } from '../../services/geofencesvc/geofencesvc_expiry_utils.js';
...
export default class GeofenceHdlr {
        ...
        try {
            const createGeofenceSchema = z.object({
                ...
                rule: z.object({
                    ...
                    // ✅ EXPIRY FEATURE INPUT (added)
                    expiryDuration: z.enum(['1_day', '1_week', '1_month']).optional(),
                }),
            ...
            // ✅ EXPIRY FEATURE COMPUTE (added)
            let expiry_at = null;
            try {
                expiry_at = computeExpiryAt(rule?.expiryDuration);
            } catch (e) {
                // e.errcode = INVALID_EXPIRY_DURATION (from utils)
                throw e;
            }
            rule.expiry_at = expiry_at;
            delete rule.expiryDuration;

            const result = await this.geofenceHdlrImpl.CreateGeofenceWithRuleLogic(
                accountid,
                userid,
                fleetid,
                geofencename,
    ...
    UpdateRule = async (req, res, next) => {
        try {
            ...
            const { fleetid, ruleid, rulename = null, ruletypeid = null, meta = null, rulegeoinfo = null, expiryDuration = null } =
                this.validateAllInputs(updateRuleSchema, req.body);

                let expiry_at = null;
                if(expiryDuration !== null && expiryDuration !== undefined){
                expiry_at = computeExpiryAt(expiryDuration);
            }

            // NOTE: we are NOT deleting anything here; we just pass expiry_at forward
            const result = await this.geofenceHdlrImpl.UpdateRuleLogic(
                accountid,
                userid,
                fleetid,
                ruleid,
