...
export default class GeofenceHdlrImpl {
    ...
    async UpdateRuleLogic(accountId, userId, fleetId, ruleId, ruleName, ruleTypeId, meta, ruleGeoInfo, permissions, expiry_at = null) {
        try {
            const isValid = await this.UserFleetValidationLogic(accountId, userId, fleetId);
            if (!isValid) {
                throw {
            ...
            return await this.geofenceSvcI.UpdateRule(accountId, userId, fleetId, ruleId, ruleName, ruleTypeId, meta, ruleGeoInfo, expiry_at);
        } catch (error) {
            throw error;
        }
    }
