...
import { computeExpiryAt } from './geofencesvc_expiry_utils.js';
...
export default class GeofenceSvcDB {
    ...
    async updateRule(accountId, userId, fleetId, ruleId, ruleName, ruleTypeId, meta, ruleGeoInfo, expiry_at = null) {
        try {
            const currentTime = new Date();
            ...
            if(expiry_at !== null && expiry_at !== undefined){
                setClauses.push(`expiry_at = $${paramIndex}`);
                setValues.push(expiry_at);
                paramIndex++;
            }
