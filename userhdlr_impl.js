...
export default class UserHdlrImpl {
  ...
  // Same style as old Driver Nemo app
  // ===========================

  checkNewUserFeedbackEligibility = async (userid) => {
    try {
      const userDetails = await this.userSvcI.GetUserDetails(userid);
  ...
  GetUserFeedbackStatusLogic = async (userid) => {
    try {
      const feedbackData = await this.userSvcI.GetLatestRatingFeedback(
        userid,
        "Nemo3.0"
      );

      if (!feedbackData) {
        return await this.checkNewUserFeedbackEligibility(userid);
      }

      const createdAt = feedbackData.created_at
        ? new Date(feedbackData.created_at)
        : null;
      ...
      // old app behavior:
      // Later => rating = "0" => show again after 30 days
      if (String(feedbackData.rating) === "0") {
        return {
          show_popup: diffInDays >= 30,
        };
      }

      // rated 1..5 => show again after 60 days
      return {
        show_popup: diffInDays >= 60,
      };
    } catch (error) {
      throw error;
    }
    ...
    try {
      ...
      if (normalizedType === "ratings") {
        ...
        if (!["0", "1", "2", "3", "4", "5"].includes(ratingValue)) {
          const error = new Error(
            "Rating must be a string value between 0 and 5"
          );
